...
class Lattice():
  ...
  def __init__(self, aspect_ratio, scale_D, dims, wavenumber, radius=1, n_substeps=None):
    #material parameters
  ...
  def random_initialize(self):
    ...
    for z_index in range(self.z_len):
      for th_index in range(self.th_len):
        ...
        self.K1s[z_index, th_index] = K1(alpha2)
        self.K3s[z_index, th_index] = K3(alpha2)
        self.director[z_index, th_index] =  random.uniform(0,2*math.pi)
    #fill derivatives
    for z_index in range(self.z_len):
      for th_index in range(self.th_len):    
        nz = cmath.polar(1, director[z_index, th_index)).real  
        nth= cmath.polar(1, director[z_index, th_index)).imag  
        leftnz = cmath.polar(1, director[z_index-1, th_index)).real  
        leftnth= cmath.polar(1, director[z_index-1, th_index)).imag  
        downnz= cmath.polar(1, director[z_index, th_index-1)).real  
        downnth= cmath.polar(1, director[z_index, th_index-1)).imag  
  ...
  def step_director_loc(self, index_z, index_th, shape, sampling_width, me):
    ...
    #get K1, K3
    K1 = K1s[index_z, index_th]
    K3 = K3s[index_z, index_th]
